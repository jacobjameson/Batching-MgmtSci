...
More specifically, he Mayo Clinic ED employs a sophisticated
...
system automatically assigns patients completely on a rotational basis
to physicians 60 seconds after registration through the electronic
health record system. At shift start, each physician receives four
consecutive patients to establish an initial patient load, after which
they enter rotation with other on-duty physicians. The rotation order is
predetermined by the ED scheduler and varies across shifts to ensure
fairness over time. Critically, these assignments are based solely on
arrival time---the algorithm does not consider patient demographics,
chief complaint, Emergency Severity Index score, physician workload, or
the acuity of recently assigned patients. To maintain system integrity,
physicians receive no new patients during their final 120 minutes and
are capped at 18 patients per shift.
...
imaging modalities ordered within a 5-minute window at the start of a
patient \citep[\citet{jameson2024variation}]{su2025crisis}. We focus on
early batching (within 5 minutes) because this represents the moment of
...
ex-ante which patients will ultimately require multiple tests, making
early batching a discretionary choice based on practice style rather
than clinical necessity. Each imaging modality, such as X-ray, contrast
CT scan, non-contrast CT, and ultrasound, is considered a separate and
distinct test for our study. In particular, we focus on batching
instances where the physician orders different imaging tests because
such tests cannot be done in a single scanning session (due to
...
(ED capacity guidelines for Mayo Clinic are in Table EC.2.1 The variable
\(Batched_i\) may be endogenous; for example, injury severity may be
unobserved and correlated with the need to run multiple tests, length of
stay, and the likelihood of returning with admission. Hence, we
instrument \(Batched_i\) with the assigned physician \(j\)'s underlying
tendency to batch, \(BatchTendency_{i,j}^{phys}\). Standard errors are
heteroskedasticity-robust (EC.13 provides additional details).
...
\citep{bhuller2020incarceration}. Figure EC.7.1 presents both of these
tests for various subsamples of interest. In the left panel, our
...
\subsubsection{More Details on Our LATE Estimates}\label{sec:late}
...
most justified, standard practice --- preserving diagnostic
flexibilityyields operational performance that is similar or better
without compromising care quality. These findings simplify policy
...
capacity level of the ED, defined in Table EC.2.1. \(\mathbf{X_{i}}\) is
the vector of patient covariates described in the previous section and
in Figure \ref{fig:batch_tendency}. Table \ref{tab:determinants}
...
suggest that batching leads to a 44.3\% increase in length of stay and
...
excess imaging and \$224 from excess capacity utilization. Scaling to
...
\$113,000 annually while freeing 850 bed-hours of capacity.
...
magnitudes (Figure EC.9.1). Third, we conduct a placebo test by
examining complaints in which imaging is not ordered and find null
effects, supporting the idea that batch tendency captures
batching-specific behavior (Table EC.8.1). We acknowledge that we cannot
definitively rule out all exclusion restriction violations. We note that
to the extent batch tendency captures general physician intensity, our
...
occupancy strongly influence batching, indicating that physicians adjust
orders based on workload and operational demands.
...
processes at once. This supports prior research suggesting sequential
test ordering can minimize unnecessary tests by allowing earlier results
to inform further decisions. Our findings suggest that while physician
discretion can tailor test ordering, it may also promote practices that
raise resource use and inefficiency without benefiting patients or the
ED system.
