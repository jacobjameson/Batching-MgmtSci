...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\documentclass[,,blindrev]{informs}
...
and MGH. The MGH dataset, which includes 111,710 adult patient
encounters from November 10, 2021, through December 10, 2022, provides a
robust sample for validating the generalizability of our findings. Our
primary analysis, however, focuses on the Mayo Clinic data due to its
unique random patient-physician assignment, which enables stronger
causal inference. Specifically, the Mayo Clinic ED employs a
computerized rotational patient assignment algorithm that addresses many
empirical challenges in healthcare settings; see, e.g.,
\citet{Traub2016}, \citet{traub2016emergency}, \citet{Traub2018}. The
system assigns patients on a completely rotational basis to physicians
60 seconds after registration in the electronic health record system. At
shift start, each physician receives four consecutive patients to
establish an initial load, after which they enter rotation with other
on-duty physicians. The rotation order is predetermined by the ED
scheduler and varies across shifts to ensure fairness over time.
Critically, these assignments are based solely on arrival time---the
algorithm does not consider patient demographics, chief complaint,
Emergency Severity Index score, physician workload, or the acuity of
recently assigned patients. To maintain system integrity, physicians
receive no new patients during their final 120 minutes and are capped at
18 patients per shift.
...
errors are heteroskedasticity-robust (EC.12 provides additional
details).
...
likely to have their tests
batched.\footnote{The coefficient is greater than one because the BatchTendency instrument has a narrow range centered near zero (10th to 90th percentile spans only 0.047 units).}
A one-unit change in the instrument is therefore far outside the
observed range, making per-unit interpretation uninformative. Instead,
we interpret the effect over realistic variation: moving from the 10th
to 90th percentile of batch tendency increases batching probability by
8.2 percentage points, as shown above.
...
complaints (EC.7) and various robustness checks in Section
\ref{sec:robustness}, including sensitivity analyses (EC.8) that allow
...
that same subsample \citep{bhuller2020incarceration}. Figure EC.6.1
...
batch tendency. EC.7 attempts to distinguish between and identify the
...
\ref{tab:results_table}), and each additional test is associated with a
\(12.4\%\) increase in LOS conditional on
...
ED, we replicated our analysis using data from the Massachusetts General
Hospital (MGH) ED, one of the busiest emergency departments in the
United States. The MGH dataset comprises 111,710 adult patient
encounters from November 2021 through December 2022 (. After applying
analogous sample restrictions---retaining adult patients with imaging
orders, excluding low-volume providers and rare complaints---the
analytical sample comprises 26,875 encounters. Full details on the MGH
data, sample construction, and complete regression tables are provided
in EC.13.
...
into different care areas (e.g., urgent care, acute care, observation)
...
bias, we adjust our instrumental variable strategy by including care
area fixed effects in both stages of our 2SLS estimation and in the
construction of the batch tendency instrument, thereby accounting for
the sorting of patients into different ED zones. While this approach
cannot guarantee the same level of causal identification as Mayo
Clinic's randomized system, it provides a meaningful test of whether the
relationship between batching and patient outcomes replicates in an
institutionally distinct setting.

Figure \ref{fig:mgh} presents 2SLS estimates with full controls from
both sites. The results are broadly consistent: discretionary batching
significantly increases the number of imaging tests ordered at MGH
(1.372 additional tests, \(p < 0.001\)) and increases log ED length of
stay (0.532, \(p < 0.05\)), with no significant effect on 72-hour
returns with admission (\(-0.042\), \(p > 0.10\)). The first-stage
F-statistics at MGH are 41.2 and 36.2 with necessary and full controls,
respectively, indicating a strong instrument. To formally assess whether
the estimated effects differ across the two sites, we conduct
independent-samples \(z\)-tests:

\[
z = \frac{\hat{\beta}_{\text{Mayo}} - \hat{\beta}_{\text{MGH}}}{\sqrt{\text{SE}_{\text{Mayo}}^2 + \text{SE}_{\text{MGH}}^2}}
\]

which is valid because the two samples are non-overlapping. As shown in
Figure \ref{fig:mgh}, none of the four primary outcomes exhibit
statistically significant differences across sites (\(p > 0.55\) for
all), indicating that the effects of batching are statistically
indistinguishable between Mayo Clinic and MGH.

Overall, these results reinforce the external validity of our core
findings. The effects of discretionary batching on ED efficiency and
imaging volume are not artifacts of a single institution's workflow but
a systematic consequence of batch ordering in high-volume emergency care
settings.
...
remain small and statistically insignificant (Table EC.10.2). For
...
affected by batching. For example, we observe a 40 percentage-point
