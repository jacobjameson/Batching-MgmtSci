...
\citet{Daniels1977}]{Miller1994}. However, relatively research has
considered how to manage test ordering strategies when this discretion
exists.
...
\subsection{Challenges and Empirical Strategy}\label{sec:challenges}

Though the importance of understanding batching behavior in EDs is
...
non-random patient-to-physician assignment. We do so by controlling for
care area assignment and acuity to account for non-random patient
sorting and to investigate the validity of our main findings across two
...
subset of patients whose exposure to batching depends on the physician
to whom they are randomly assigned. This is particularly important in
the context of the ED, where there are clear cases where batch ordering
is necessary (e.g., when multiple tests are essential and almost every
...
boarding; results are qualitatively unchanged and are reported in EC.11.
...
broader any-return measure in EC.11.
...
The mediation analysis results, detailed in Tables EC.5.1 and EC.5.2,
...
Tables EC.5.1 and EC.5.2. This suggests that the delays attributed to
...
\ref{tab:results_table}, the SEM analysis (Table EC.5.1) revealed that
...
(Tables \ref{tab:results_table}, EC.5.1, and EC.5.2) reinforces the
conclusion that diagnostic intensity and admission are the key drivers
of increased LOS among batched patients.
...
non-significant (\(c^\prime = -0.100\), \(p = 0.216\)). This further
...
magnitudes (Figure EC.9.1). Third, we conduct a placebo test by applying
...
outcomes in this placebo sample (Table EC.8.1), supporting the idea that
...
(Table EC.11.1). For 72-hour returns, regardless of admission, effects
...
EC.7.1). We consistently find stable effect sizes and implications
...
hospitalization. As shown in Table EC.12.1, the estimated effect of
...
Combining our causal estimates with published cost parameters (see EC.6
...
ED (Table EC.6.1). These estimates represent conservative lower bounds,
