...
\noindent\textbf{Response:} We thank the reviewer for this valuable suggestion. We have developed a cost-benefit analysis that quantifies the operational and financial burden of discretionary batch ordering. 
...
We estimate imaging costs using the 2024 Medicare Physician Fee Schedule, combining professional and facility components. Our UJIVE estimates (Table 4, Panel B) indicate that discretionary batching significantly increases X-ray utilization (0.96 additional tests, $p < 0.001$), while effects on other modalities (ultrasound, CT) are not statistically significant. To maintain a conservative approach, we include only the statistically significant X-ray effect in our primary analysis. Using Medicare rates of \$60--100 per X-ray (mean \$80), we estimate excess imaging costs of \textbf{\$77} (\$58--96) per batched complier.

To value the excess ED bed-hours consumed by discretionary batching, we draw on published estimates of ED operational costs. Recent time-driven activity-based costing studies report ED bed-hour costs of \$58--77 \citep{schreyer2017economics, canellas2024measurement}. Adjusting for inflation and regional variation, we use a range of \$75--125 per bed-hour (mean \$100). Our UJIVE estimates indicate discretionary batching increases LOS by 65\% (Table \ref{tab:results_table}), translating to 158 additional minutes (2.64 hours) relative to baseline LOS of 242 minutes for patients receiving standard care. At \$75--125 per bed-hour, this yields \textbf{\$264} (\$198--330) in excess capacity costs per batched complier.

Table XX summarizes our findings. Combining imaging and capacity costs, discretionary batching imposes an estimated \textbf{\$341} (\$255--426) in direct costs per affected patient. Capacity costs dominate (77.5\% of total), reflecting the substantial LOS increase we document.

Scaling to annual ED operations, discretionary batching costs approximately \textbf{\$227,000 per year} for a medium-sized ED (40,000 annual visits) and \$340,000 for a large urban ED (60,000 visits). For our study site, the annual cost is approximately \$221,000. These costs translate to meaningful operational impacts: eliminating discretionary batching would free approximately 1,700 bed-hours annually at our study site---equivalent to 72 additional 24-hour days of ED capacity---enabling approximately 425 additional patient visits (a 1.1\% capacity improvement).
...
\begin{tabular}{lcccc}
\toprule
ED Type & Annual Volume & Batched Compliers & Annual Cost & 95\% Range \\
\midrule
Small (Rural/Community) & 20,000 & 333 & \$113,436 & (\$85,077--\$141,795) \\
Medium (Suburban) & 40,000 & 666 & \$226,872 & (\$170,154--\$283,590) \\
Large (Urban) & 60,000 & 999 & \$340,308 & (\$255,231--\$425,386) \\
Study Site (Mayo Clinic) & 39,083 & 650 & \$221,422 & (\$166,066--\$276,777) \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\footnotesize
\item \textit{Notes:} Annual costs calculated as batched compliers $\times$ \$341 per patient. Batched compliers estimated as 23.9\% of ED volume (imaging-relevant encounters) $\times$ 7.0\% (batched complier share of imaging encounters). Imaging costs include only statistically significant X-ray effects (\$77 per patient); including point estimates for other modalities would increase costs by approximately 25\%. Capacity costs (\$264 per patient) assume \$100/bed-hour and 2.64 additional bed-hours per batched complier. Range reflects uncertainty in imaging (\$60--100/X-ray) and bed-hour (\$75--125/hour) cost parameters.
\end{tablenotes}
\end{threeparttable}
\end{table}

Several factors suggest these estimates represent lower bounds on the true cost of discretionary batching:

\begin{enumerate}
    \item \textit{Imaging costs include only statistically significant effects.} Including point estimates for ultrasound and CT (Table \ref{tab:results_table}, Panel B) would increase imaging costs from \$77 to \$163 per patient, raising total costs by 25\%.
    
    \item \textit{Excluded direct cost categories.} We do not include radiologist interpretation time, physician cognitive burden, or patient time costs (waiting, lost productivity).
    
    \item \textit{Downstream costs from increased admissions.} Our finding that discretionary batching increases admission probability by 40 percentage points (Table \ref{tab:results_table}, Panel C) implies substantial downstream costs we do not quantify. Average inpatient stays cost \$10,000--15,000; even if only a fraction of additional admissions represent overuse, the financial implications would substantially exceed our direct cost estimates.
    
    \item \textit{Spillover effects.} Extended LOS creates congestion externalities that delay care for other patients, imposing costs we do not capture.
    
    \item \textit{Throughput opportunity costs.} Our bed-hour valuation uses operational costs rather than throughput value. If freed bed-hours enable additional patient visits valued at \$500--1,500 each \citep{feizi2025vertical}, the capacity benefits would be substantially larger.
\end{enumerate}

Even under conservative assumptions, discretionary batch ordering imposes meaningful costs---approximately \$227,000 annually for a medium-sized ED---without corresponding improvements in patient outcomes (Table \ref{tab:results_table} shows no significant effects on 72-hour returns). These costs arise purely from the approximately 20\% of patients whose batching decisions reflect physician preference rather than clinical necessity.

These estimates suggest that relatively modest interventions---such as decision support tools encouraging sequential ordering when clinically appropriate, or providing physicians with feedback on their batching rates relative to peers---could yield meaningful cost savings. A 50\% reduction in discretionary batching would save approximately \$110,000 annually while freeing 850 bed-hours. Given that compliers represent patients at the margin of physician discretion (not those with clear clinical indications for comprehensive imaging), interventions targeting this group are unlikely to compromise care quality while offering substantial operational benefits.

We have added this analysis as Section 5.X and Appendix Table EC.X in the revised manuscript.
