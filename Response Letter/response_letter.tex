...
Second, we added the Unbiased Jackknife IV Estimator (UJIVE), following \citet{goldsmithpinkham2025leniencydesignsoperatorsmanual}, which is purpose-built for leniency designs with many decision-makers. When 2SLS uses many physician indicators as instruments, it constructs estimated leniency partly from each patient's own treatment status, creating mechanical correlation with the outcome error that biases estimates toward OLS. UJIVE eliminates this bias through leave-one-out estimation, constructing each patient's predicted treatment using only other patients' data. The close agreement between our 2SLS results (using a single batch tendency instrument) and UJIVE estimates (using physician indicators directly) provides important validation: it indicates that many-instrument bias is not driving our findings and that both approaches identify similar underlying variation. Across all outcomes, UJIVE estimates closely match our 2SLS results under the preferred full-control specification:

\begin{itemize}
\item 0.447 increase in log treatment time ($\approx$ 56\%)
\item 0.522 increase in log time to disposition ($\approx$ 69\%)
\item 0.503 increase in log LOS ($\approx$ 65\%)
\item 1.174 additional imaging tests ($\approx$ 88\%)
\end{itemize}

Third, we clarified the theoretical framework for discretionary batch ordering. The revised Sections 2.3 and 3.3 explicitly distinguish between clinically mandated imaging and the discretionary margin where physician practice style determines the approach. Our identification strategy targets this margin, by focusing on ``compliers'' and estimating LATE values, and the complier analysis confirms that physician assignment affects testing strategy for approximately 20\% of patients---precisely the subgroup relevant for operational policy.
...
\begin{figure}[t!]
\centering
\caption*{Figure 1: Physician Variation in Batch Ordering Imaging Tests}
...
\begin{table}[t!]
\centering
\caption*{Table 2: First-Stage Results: Batch Tendency and Batching}
\begin{threeparttable}
\begin{tabular}{lcc}
\toprule 
...
To complement our 2SLS specification, we also incorporate the Unbiased Jackknife Instrumental Variables Estimator (UJIVE), following \cite{goldsmithpinkham2025leniencydesignsoperatorsmanual}. UJIVE addresses a specific concern when using many physician indicators as instruments: 2SLS constructs estimated leniency partly from each patient's own treatment status, creating mechanical correlation with the outcome error that biases estimates toward OLS. UJIVE eliminates this many-instrument bias through leave-one-out estimation. The agreement between our 2SLS results (using a single batch tendency instrument) and UJIVE estimates (using physician indicators directly) provides important validation that many-instrument bias is not driving our findings.
...
\toprule
& Standard care & \multicolumn{2}{c}{\underline{2SLS}} & \multicolumn{2}{c}{\underline{UJIVE}} \\
& mean & (2) & (3) & (4) & (5) \\
\midrule
...
The figure below (now included in EC.9) presents sensitivity analyses showing substantial robustness. For all outcomes, the treatment effect remains significant across $\delta \in [-0.2, 0.2]$.

\begin{figure}[t!]
\centering
\caption*{Figure EC.9.1: Sensitivity of Treatment Effect Estimates to Exclusion Restriction Violations}
...
\begin{flushleft}
\footnotesize
\textit{Notes:} Column 1 reports means for standard care patients (standard deviations in parentheses). 
...
Regarding cost-benefit analysis, we developed a cost-benefit analysis quantifying the operational and financial burden of discretionary batching (detailed in our response to Reviewer 2 and EC.6). Under deliberately conservative assumptions, which include include only statistically significant imaging effects and excluding downstream costs from increased admissions, discretionary batching imposes approximately \$349 in direct costs per affected patient: \$125 from excess imaging and \$224 from excess ED capacity utilization. Scaling to annual operations, this amounts to approximately \$232,000 per year for a medium-sized ED (40,000 annual visits). These costs arise from the approximately 20\% of patients whose batching decisions reflect physician preference rather than clinical necessity, with no offsetting benefits in 72-hour return rates. Even a 50\% reduction in discretionary batching would save approximately \$113,000 annually while freeing 850 bed-hours of capacity.
...
Reviewer 3 asked for a clearer discussion of representativeness. We now provide a CONSORT-style flow diagram (Figure EC.3.1) detailing all exclusion steps. Table EC.3.2 compares included and excluded encounters on key demographics, vital signs, and complaint categories. As discussed in Section 3.2, the analytic sample appropriately focuses on higher-acuity patients whose presenting complaints plausibly require imaging. This is the clinically meaningful population where discretionary batch ordering has operational consequences. We characterize representativeness in that domain rather than for the ED population as a whole, which we now state explicitly. We additionally characterize compliers in EC.4, confirming that they span a broad range of demographics and complaint types, enhancing external validity.

\textbf{4. Physician fixed effects and model selection}

The AE and Reviewer 3 both raised the need for robustness to unobserved provider characteristics. As noted earlier, we now incorporate the Unbiased Jackknife Instrumental Variables Estimator (UJIVE) into our main robustness analysis. UJIVE uses the full set of physician identifiers as instruments and eliminates the many-instrument bias that arises when 2SLS constructs estimated leniency partly from each patient's own treatment status. The close agreement between our 2SLS and UJIVE estimates—despite their very different constructions—indicates that many-instrument bias is not driving our findings. Our expanded controls and Conley sensitivity analysis address the exclusion restriction concerns highlighted by the AE directly. With full controls, the UJIVE estimates are slightly attenuated but remain large and precise (e.g., log LOS: 0.503; time to disposition: 0.522; treatment time: 0.447), reinforcing that the effects we identify are not artifacts of unobserved provider heterogeneity.

We also addressed the concerns about model selection raised by Reviewer 3. EC.10 now includes logit models for binary outcomes and negative binomial models for count outcomes. Across all outcomes, the resulting average marginal effects align closely with those from our linear IV specifications. We retain linear 2SLS in the main text because nonlinear IV models violate core econometric requirements (the ``forbidden regression''), a point we clarify in our detailed response to Reviewer 3. The combination of (i) FE-based IV, (ii) UJIVE estimation, and (iii) nonlinear outcome models provides strong evidence that our findings are robust to alternative estimators and modeling choices.

\textbf{4. Alternative outcomes}

We now additionally examine outcomes reflecting more proximate process measures: treatment time (provider contact to decision) and time to disposition (provider contact to disposition decision). These outcomes remove waiting room delays and post-disposition boarding. EC.11.1 reports that the pattern remains consistent across all three measures: batch ordering increases treatment time by 56\%, time to disposition by 69\%, and total LOS by 65\%. Because each metric captures a distinct operational stage of the encounter, we retain all of them in Table 4 and EC.11 to provide a more complete characterization of the effects.
...
Our revised main results (Table 4) now incorporate the expanded controls and additional methods we employed that are more robust to physician-level unobserved heterogeneity. Specifically, In addition to our standard 2SLS estimates using batch tendency as the instrument, Table 4 includes estimates from the Unbiased Jackknife Instrumental Variables Estimator (UJIVE). We have added this estimator because it is purpose-built for leniency designs with many decision-makers. When 2SLS uses many physician indicators as instruments, it constructs estimated leniency partly from each patient's own treatment status, creating mechanical correlation with the outcome error that biases estimates toward OLS. UJIVE eliminates this many-instrument bias through leave-one-out estimation. The agreement between our 2SLS and UJIVE estimates indicates that many-instrument bias is not driving our findings and that both approaches identify similar underlying variation.
...
The attenuation in the 2SLS estimates after adding precision controls confirms the reviewer’s intuition that our original instrument may have contained some correlation with general diagnostic intensity. At the same time, the close agreement between the 2SLS and UJIVE estimates---both before and after adjustment---provides strong evidence that any remaining correlation between a physician’s overall comprehensiveness and patient outcomes is too small to materially bias the results. The close agreement between 2SLS and UJIVE estimates, despite their different constructions of identifying variation, indicates that many-instrument bias is not driving our results. This convergence, combined with our expanded controls for laboratory ordering and physician characteristics, strengthens confidence that our revised specification isolates batching-specific effects.
...
We have revised the manuscript to reflect these more conservative estimates while maintaining transparency about our identification assumptions. The core finding remains: discretionary batch ordering increases ED length of stay by approximately 65\% without improving short-term patient outcomes, suggesting substantial opportunity for operational improvement through policies that preserve diagnostic flexibility.
...
Third, as mentioned earlier, we have added formal sensitivity analysis to rigorously test the potential impact the violation of exclusion restriction might have on our main findings (EC.9). Finally, have revised Section 4.9 (Limitations) to acknowledge this concern more directly, removing claims that violations ``may likely only have a small impact'' and instead transparently presenting our validation evidence through expanded controls, while acknowledging that definitive exclusion restriction tests are not possible in observational settings.
...
\begin{flushleft}
\footnotesize
\textit{Notes:} This table reports instrumental-variables estimates of the effect of batching on the number of distinct imaging tests among patients discharged from the emergency department. 
Columns (1)–(2) report 2SLS estimates using physician batch tendency as the instrument. 
Columns (3)–(4) report UJIVE estimates using ED provider identifiers as many weak instruments. 
$^{***} p < 0.001$.
\end{flushleft}
...
We emphasize that this discharged-only analysis should be interpreted with caution, as discharge status is itself a downstream outcome potentially affected by ED imaging decisions. Conditioning on discharge therefore risks post-treatment selection and may attenuate or otherwise distort causal estimates. For this reason, we do not treat this subsample analysis as a primary causal estimate. Rather, it serves as a robustness check demonstrating that the positive association between batching and imaging intensity is not driven solely by patients who are ultimately admitted and receive further inpatient testing. We report this analysis in EC.12 and reference it here solely to clarify interpretation of our findings in response to the reviewer’s concern about frontloading.
...
\noindent\textbf{Response:}\color{blue} We thank the reviewer for this valuable suggestion. We have developed a cost-benefit analysis that quantifies the operational and financial burden of discretionary batch ordering, adopting a societal perspective with Medicare-allowed amounts as proxies for resource costs.

Our analysis focuses on two cost components where we have direct causal evidence: (1) excess imaging costs from additional tests and (2) excess ED capacity consumed from extended length of stay. We deliberately adopt conservative assumptions throughout, excluding several additional cost categories---including physician time, radiologist interpretation burden, patient time costs, downstream costs from increased admissions, and spillover congestion effects---making our estimates lower bounds on the true cost of discretionary batching.

A key input to our analysis is identifying the population affected by discretionary batching. Our complier analysis (EC.4) indicates that approximately 20\% of patients in our analytical sample are ``compliers''---patients whose batching status depends on physician assignment rather than clinical necessity. Among the 1,595 batched patients in our sample, roughly half (51\%) are compliers whose batching reflects physician discretion rather than clinical indication. These 813 ``batched compliers'' in our 15-month sample (650 annualized) represent the patients who would benefit from interventions targeting discretionary batching.

We estimate imaging costs using Medicare-allowed reimbursement for diagnostic radiography in the ED setting. For a two-view chest radiograph (CPT 71046), the technical component under HOPPS is \$98.53, with an additional \$30--40 for the professional component, yielding total Medicare-allowed reimbursement of approximately \$120--140 per study \citep{dabus2025practice}. Our UJIVE estimates (Table 4, Panel B) indicate that discretionary batching significantly increases X-ray utilization (0.96 additional tests, $p < 0.001$), while effects on other modalities (ultrasound, CT) are not statistically significant. To maintain a conservative approach, we include only the statistically significant X-ray effect in our primary analysis. Using a range of \$100--160 per X-ray (mean \$130), we estimate excess imaging costs of \textbf{\$125} (\$96--153) per batched complier.

To value the excess ED bed-hours consumed by discretionary batching, we draw on published estimates of ED operational costs using time-driven activity-based costing. \citet{schreyer2017economics} report ED personnel costs of \$58.20 per bed-hour (2010--2011 data; approximately \$83 inflation-adjusted to 2024), while \citet{canellas2024measurement} find daily ED boarding costs of \$1,856, implying approximately \$77 per hour. Based on these estimates, we use a range of \$75--100 per bed-hour (mean \$85). Our UJIVE estimates indicate discretionary batching increases LOS by 65\% (Table \ref{tab:results_table}), translating to 158 additional minutes (2.64 hours) relative to baseline LOS of 242 minutes for patients receiving standard care. At \$75--100 per bed-hour, this yields \textbf{\$224} (\$198--264) in excess capacity costs per batched complier.

Table EC.6.1 summarizes our findings. Combining imaging and capacity costs, discretionary batching imposes an estimated \textbf{\$349} (\$294--417) in direct costs per affected patient. Capacity costs account for 64\% of the total, reflecting the substantial LOS increase we document.

Scaling to annual ED operations, discretionary batching costs approximately \textbf{\$232,000 per year} for a medium-sized ED (40,000 annual visits) and \$349,000 for a large urban ED (60,000 visits). For our study site, the annual cost is approximately \$227,000. These costs translate to meaningful operational impacts: eliminating discretionary batching would free approximately 1,700 bed-hours annually at our study site---equivalent to 72 additional 24-hour days of ED capacity---enabling approximately 425 additional patient visits (a 1.1\% capacity improvement).

\begin{table}[t!]
\centering
\caption*{Table EC.6.1: Estimated Annual Cost of Discretionary Batch Ordering by ED Size}
...
\begin{tablenotes}
\footnotesize
\item \textit{Notes:} Annual costs calculated as batched compliers $\times$ \$349 per patient. Batched compliers estimated as 23.9\% of ED volume (imaging-relevant encounters) $\times$ 7.0\% (batched complier share of imaging encounters). Imaging costs (\$125 per patient) include only statistically significant X-ray effects; including point estimates for other modalities would increase costs by approximately 17\%. Capacity costs (\$224 per patient) assume \$85/bed-hour and 2.64 additional bed-hours per batched complier. Range reflects uncertainty in imaging (\$100--160/X-ray) and bed-hour (\$75--100/hour) cost parameters.
\end{tablenotes}
\end{threeparttable}
\end{table}

Even under conservative assumptions, discretionary batch ordering imposes meaningful costs---approximately \$232,000 annually for a medium-sized ED---without corresponding improvements in patient outcomes (Table 4 shows no significant effects on 72-hour returns). These costs arise purely from the approximately 20\% of patients whose batching decisions reflect physician preference rather than clinical necessity.

These estimates suggest that relatively modest interventions---such as decision support tools encouraging sequential ordering when clinically appropriate, or providing physicians with feedback on their batching rates relative to peers---could yield meaningful cost savings. A 50\% reduction in discretionary batching would save approximately \$116,000 annually while freeing 850 bed-hours. Given that compliers represent patients at the margin of physician discretion (not those with clear clinical indications for comprehensive imaging), interventions targeting this group are unlikely to compromise care quality while offering substantial operational benefits.

We have added a detailed cost analysis in EC.6, including the above table and discussion. We believe this addition substantially enhances the practical relevance of our findings for ED administrators considering policies to influence physician batching behavior. We thank the reviewer for prompting us to include this important analysis and have referenced it in the main text (Section 4.7), included here for posterity:

\begin{quote}
``Combining our causal estimates with published cost parameters (see EC.6 for details), we adopt a societal perspective using Medicare-allowed reimbursement of \$130 per ED radiograph \citep{dabus2025practice} and ED bed-hour costs of \$85 based on time-driven activity-based costing studies \citep{schreyer2017economics, canellas2024measurement}. Under deliberately conservative assumptions, discretionary batching imposes at least \$349 in direct costs per batched complier: \$125 from excess imaging and \$224 from excess capacity utilization. Scaling to annual operations, this amounts to approximately \$232,000 per year for a medium-sized ED (40,000 annual visits) and \$349,000 for a large urban ED (Table EC.6.1). These estimates represent conservative lower bounds, include only statistically significant imaging effects (excluding ultrasound and CT point estimates), value bed-hours at operational cost rather than throughput value, and exclude downstream costs from the 40 percentage point increase in admission probability. Inpatient stays cost \$10,000–15,000, which would dwarf our direct cost estimates. Notably, we find no offsetting benefits: 72-hour return rates are statistically indistinguishable between strategies. Even under these conservative assumptions, a 50\% reduction in discretionary batching would save about \$113,000 annually while freeing 850 bed-hours of capacity.''
\end{quote}

We appreciate the reviewer's suggestion, which has led us to provide a more comprehensive assessment of the operational and financial implications of discretionary batching in EDs.
\color{black}
...
\begin{quote}
``Discretionary batching also leads to more intensive diagnostic testing. The marginal batched patient receives 1.2 additional imaging tests (completed studies with documented results), representing an 88\% increase from the mean for patients managed through standard practice.''
\end{quote}
...
Our revised main results (Table 4) now incorporate the expanded controls and respond to the reviewers suggestion to employ methods that use physician FE (which we describe in (3)). In addition to our standard 2SLS estimates using batch tendency as the instrument, Table 4 includes estimates from the Unbiased Jackknife Instrumental Variables Estimator (UJIVE), following \cite{goldsmithpinkham2025leniencydesignsoperatorsmanual}. UJIVE addresses a specific econometric concern: when 2SLS uses many physician indicators as instruments, it constructs estimated leniency partly from each patient's own treatment status, creating mechanical correlation with the outcome error that biases estimates toward OLS. UJIVE eliminates this many-instrument bias through leave-one-out estimation. We emphasize that UJIVE, like all IV estimators, still requires the exclusion restriction to hold—our expanded controls and Conley sensitivity analysis address exclusion restriction concerns directly. The agreement between 2SLS and UJIVE estimates provides validation that many-instrument bias is not driving our findings
...
\item UJIVE estimates, which eliminate many-instrument bias, closely match 2SLS results
\item 2SLS estimates remain robust to potential backdoor paths for plausibly large exclusion restriction violations
\end{itemize}
...
By performing leave-one-out estimation of each physician’s effect, UJIVE removes the influence of stable, provider-level traits from the instrument, isolating variation driven by the random patient assignment mechanism. While UJIVE does not solve the exclusion restriction; rather, it provides a rigorous robustness check that corrects the potential many-instrument bias of 2SLS. The agreement between UJIVE and 2SLS estimates—despite their very different constructions—indicates that many-instrument bias is not driving our findings and that both approaches identify similar underlying variation.
...
We have added a discussion in Section 3.4 (Empirical Specification) explaining our choice of linear models in the IV framework, with reference to the econometric literature. We have also added the included robustness table to EC.10, which shows the OLS robustness check with nonlinear models.
...
Figure EC.3.1 (EC.3) shows the complete sample selection process:

\begin{figure}[t!]
\centering
\begin{threeparttable}
\includegraphics[width=0.8\textwidth]{../outputs/figures/CONSORT.png}    
\caption*{Figure EC.3.1: CONSORT Flow Diagram for Sample Selection}    
\label{fig:consort}    
\begin{tablenotes}
\small
\item \textit{Notes:} Sample selection for Mayo Clinic ED encounters (October 2018 - December 2019). Starting with 48,854 encounters, we apply sequential exclusion criteria to arrive at our analytical sample of 11,697 encounters. Rare complaints have fewer than 1,000 total encounters. Low-batching complaints have batching rates below 5\%.
\end{tablenotes}
\end{threeparttable}
\end{figure}

Starting with 48,854 encounters of adult patients, we exclude: (1) encounters with non-full time providers, (2) rare complaints ($<1000$ encounters), (3) complaints where batching occurs $<$5\% of the time, and (4) complaints that do not involve any imaging. This yields our analytical sample of 11,679 encounters (23.9\%). We made this decisions based on conversations and discussions with our medical collaborators (and co-authors on this paper) who are in charge of operations in two leading US hospitals (Mayo Clinic and MGH).

The table included below (now included in EC.3) compares patient characteristics and outcomes between excluded and included encounters. As can be seen, the  analytical sample differs from excluded encounters in clinically meaningful and expected ways. Included patients are older (62.9 vs. 57.5 years), higher acuity (ESI 2.52 vs. 2.87, where lower numbers indicate greater severity), and more likely to present with concerning vital signs (febrile: 4.6\% vs. 1.4\%, hypotensive: 2.3\% vs. 1.2\%). Consequently, they experience longer ED stays (272 vs. 238 minutes) and higher admission rates (28.4\% vs. 17.0\%).


\begin{table}[t!]
\centering
\caption*{Table EC.3.2: Comparison of Excluded vs. Included Encounters}
\begin{threeparttable}
\begin{tabular}{lccc}
\toprule
& Excluded & Included & \\
...
\begin{tablenotes}
\footnotesize
\item \textit{Notes:} Comparison of patient characteristics and outcomes between excluded encounters (n=37,175) and analytical sample (n=11,679). Standard deviations in parentheses for continuous variables. ESI (Emergency Severity Index) ranges from 1 (highest acuity) to 5 (lowest acuity). P-values from two-sample t-tests for continuous variables and proportion tests for binary variables. *** p<0.001.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\begin{itemize}
\item Added detailed CONSORT diagram (Figure EC.3.1)
\item Added comparison of excluded vs. included encounters (Table EC.3.2)
\item Revised Section 3.2 to clarify sample selection rationale with appropriate citations
\item Added discussion of generalizability in Section 4.7 
\end{itemize}
...
We have therefore added this analysis using treatment time (defined as the time from the first physician contact to the disposition decision). The results, shown in the table below (now in EC.11), demonstrate that our findings are robust to this alternative specification:
...
\begin{table}[t!]
\centering
\caption*{EC.11.2: Robustness: Effect of Batching on 72-Hour Return Outcomes}
\begin{threeparttable}
\begin{tabular}{lcccccc}
\toprule
& Standard care & \multicolumn{2}{c}{\underline{2SLS}} 
...
\begin{itemize}
\item Added treatment time analysis (EC.11.1)
\item Added any 72-hour return analysis (EC.11.2)
...
We have added Section 4.4, which presents this heterogeneity analysis, with Figure 4 showing coefficient plots. Section 5.2 discusses implications for tailoring diagnostic strategies, noting that the lack of heterogeneity simplifies policy recommendations.
...
Regarding Section 1.1, we have removed the dedicated subsection heading and condensed the empirical challenges and strategy discussion from three paragraphs into a single paragraph within the introduction. This paragraph now briefly highlights the key challenges (endogeneity, data requirements, and generalizability), summarizes how our setting and data address them, and directs readers to Section 3 for full details. The LATE discussion that previously appeared here has been removed, as it is covered in detail in Section 3.3.5. We believe this streamlines the introduction while preserving the essential roadmap for readers.

We have also restructured the opening of Section 3.3 to follow the recommended logical progression. The section now begins with a new framing paragraph that (1) presents our estimand and references the second-stage equation, (2) explicitly discusses the endogeneity concern—that batching decisions may correlate with unobserved patient severity—and (3) introduces physician batch tendency as our instrumental variable solution. The remainder of the section then proceeds to instrument construction, validation, and identifying assumptions. We believe this reframing makes the identification strategy immediately transparent to the reader.
