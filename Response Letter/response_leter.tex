...
We have carefully revised the paper to address all comments, and it has significantly benefited from the suggested revisions. We first briefly summarize the main changes and then provide individual responses.

\begin{enumerate}
\item We have substantially strengthened our identification strategy to address concerns about the exclusion restriction. All three reviewers and the AE raised the concern that our instrument (physician batch tendency) might capture general diagnostic intensity rather than imaging-specific ordering behavior. We address this in three ways: (a) we expanded our control set to include laboratory tests ordered, physician characteristics (experience, gender, hours into shift), and ED capacity levels; (b) we implemented the Unbiased Jackknife Instrumental Variables Estimator (UJIVE) following Goldsmith-Pinkham et al. (2025), which is specifically designed for leniency designs with many providers and addresses provider-level unobserved heterogeneity; and (c) we conducted Conley et al. (2012) sensitivity analyses demonstrating robustness to plausible exclusion restriction violations. The close agreement between our 2SLS and UJIVE estimates---both before and after adding precision controls---provides strong evidence that our revised specification isolates batching-specific effects.
\item We have clarified our theoretical framework and treatment definition. Following guidance from the AE and all three reviewers, we reframed our comparison throughout the manuscript as ``batch ordering versus standard practice" rather than "batching versus sequencing." We added a formal hypothesis development section (Section 2.3) that articulates testable predictions grounded in the cognitive load, information value, and diagnostic momentum literatures. We also clarified that our Local Average Treatment Effect (LATE) identifies effects for ``compliers"---the approximately 13\% of patients whose testing strategy depends on physician practice style rather than clinical necessity---which is precisely the margin where ED management interventions can influence practice.
\item We have added robustness checks and refined outcome measures. Following Reviewer 3's suggestion, we added treatment time (from physician contact to disposition) as an outcome, excluding both waiting room delays and post-disposition boarding. We verified robustness to nonlinear model specifications (logit for binary outcomes, Poisson for counts). We conducted heterogeneity analyses across seven chief complaint categories, finding consistent patterns of increased imaging without efficiency gains or quality improvements across all clinical scenarios. We also provided detailed sample selection documentation, including a CONSORT flow diagram and comparison of included versus excluded encounters.
\item We have updated our estimates to reflect the enhanced specification. Our revised results show that discretionary batching increases ED length of stay by approximately 65–81\% (attenuated from the original 130\% after adding precision controls), increases imaging tests by 88–105\%, and increases admission probability by 40 percentage points, with no effect on 72-hour returns. The attenuation confirms reviewers' intuition that our original specification contained some correlation with general diagnostic intensity, while the persistence of large, significant effects demonstrates that substantial imaging-specific variation remains.
\end{enumerate}
...
Regarding subsamples where identification assumptions are most likely satisfied, our complier analysis (Appendix B) characterizes the approximately 13\% of patients whose testing strategy depends on physician assignment—precisely the margin where our assumptions hold by construction. The close agreement between our 2SLS estimates (using a single batch tendency instrument) and UJIVE estimates (using the complete set of provider indicators with leave-one-out bias correction) provides strong validation. These two approaches rely on different constructions of identifying variation, yet yield similar results, increasing confidence that many-instrument bias is not driving our findings and that the underlying source of variation is robust to alternative estimation strategies. Finally, our replication at MGH---a completely different institutional setting with non-random assignment—shows directionally similar and statistically indistinguishable effects, demonstrating external validity beyond Mayo Clinic's specific operational context.

These revisions provide the granular mechanistic evidence and robust identification the DE requested, while maintaining transparency about the limitations inherent in any observational study.
...
\begin{itemize}
\item 0.447 increase in log treatment time (≈56\%)
\item 0.522 increase in log time to disposition (≈68\%)
\item 0.503 increase in log LOS (≈65\%)
\item 1.174 additional imaging tests (≈88\%)
\end{itemize}

The alignment between 2SLS and UJIVE—two estimators with distinct identifying assumptions—provides strong evidence that our results are not driven by unobserved provider-level confounding.

Third, we clarified the theoretical framework for discretionary batch ordering. The revised Sections 2.3 and 3.3 explicitly distinguish between clinically mandated imaging and the discretionary margin where physician practice style determines the approach. Our identification strategy targets this margin, and the complier analysis confirms that physician assignment affects testing strategy for approximately 13\% of patients---precisely the subgroup relevant for operational policy.

These revisions tighten the methodological foundation and make the contribution clearer: the paper identifies and quantifies the operational consequences of discretionary batch ordering using quasi-random physician assignment, rich clinical data, and modern IV estimators designed for heterogeneous providers. We believe the revised manuscript now provides the theoretical clarity and empirical rigor appropriate for Management Science.
...
Empirically, there are clinical scenarios in which batching is necessary (e.g., polytrauma requiring multiple imaging modalities for comprehensive assessment) and scenarios where it is clearly unnecessary (e.g., uncomplicated urinary tract infections). At the same time, physicians vary substantially in their propensity to batch for clinically similar patients (Figure 1), indicating that practice style plays an important role.
...
\label{tab:first_stage_two_models}
\begin{threeparttable}
\begin{tabular}{lcc}
\toprule 
...
Our revised results (Table 4) show close agreement between 2SLS and UJIVE---both with and without precision controls---which increases confidence that both estimators identify batching-specific variation.

\begin{table}[t!]
\centering
\caption*{Table 4: Effect of Batching Tests on Patient Outcomes}
\label{tab:results_iv_ujive}
\begin{threeparttable}
...
The key clarification is that our counterfactual includes both sequential multi-test encounters and encounters where only a single test is ordered. This reflects the information problem physicians face: they cannot know ex ante which patients will ultimately need multiple tests. Thus the relevant comparison is not between two fixed protocols, but between committing upfront to comprehensive imaging versus preserving the option value of information from initial results.
...
The reviewer's concern—that our instrument might capture general ``comprehensiveness" rather than batching-specific behavior was something mentioned by all three reviewers as well as the AE. The reviewer notes that if a physician has a tendency to order more imaging, they may also tend to order more labs or be more comprehensive in other ways. This is a valid concern that we take seriously.
...
The LATE we identify represents the causal effect for compliers---patients whose imaging strategy depends on physician practice style rather than clinical necessity. This is precisely the margine where ED management interventions can influence practice without constraining clinically necessary care.
...
\subsection*{Unbiased Jackknife Instrumental Variables (UJIVE) for Leniency Designs}

Our empirical setting fits the canonical ``leniency design'' or ``judge design'' framework
\citep[e.g.,][]{GoldsmithPinkhamHullKolesarLeniencyManual,Kolesar2013Late}. Emergency
department (ED) physicians differ in their propensity to order batched advanced imaging
tests. Let $j \in \{1,\dots,K\}$ index ED providers and $i$ index patient encounters. We
denote by $x_i \in \{0,1\}$ an indicator for whether encounter $i$ receives batched imaging,
by $y_i$ an outcome of interest (e.g., log length of stay), and by $w_i$ a vector of
covariates (triage characteristics, presenting complaint, time-of-day and calendar fixed
effects, etc.). Let $z_i$ be a $K$-vector of provider indicators (one provider omitted for
collinearity). The outcome and first-stage equations are
\begin{align}
  y_i &= \beta x_i + w_i' \gamma + \varepsilon_i, \label{eq:struct}\\
  x_i &= z_i' \pi + w_i' \delta + \nu_i. \label{eq:firststage}
\end{align}
We interpret $\beta$ as the causal effect of batched (vs.\ sequential) imaging on $y_i$ for a
suitably defined complier population.

A natural approach is two-stage least squares (2SLS) using the provider indicators $z_i$ as
instruments in \eqref{eq:struct}. However, when there are many providers and rich
controls $w_i$, conventional 2SLS suffers from the well-known many-instrument bias: the
constructed first-stage fitted values mechanically reuse each observation's own treatment
status, creating a small-sample correlation between the instrument and the structural error
$\varepsilon_i$. This bias can be substantial even when the first-stage F-statistic is
moderate.

The unbiased jackknife instrumental variables estimator (UJIVE) of \citet{Kolesar2013Late}
is designed to address precisely this issue in leniency designs. Let $W$ denote the $n \times
L$ matrix stacking the covariates $w_i$, and $Z$ the $n \times K$ matrix stacking the
provider indicators $z_i$. Define the residualized instruments and treatment by
\[
  \tilde Z = M_W Z, \qquad \tilde x = M_W x,
  \qquad M_W = I_n - W(W'W)^{-1}W',
\]
so that we partial out the necessary assignment controls $w_i$ from both $x_i$ and $z_i$.
Consider the population regression of $\tilde x$ on $\tilde Z$,
\[
  \tilde x_i = \tilde z_i' \pi + v_i,
\]
where $\tilde z_i$ is the $i$th row of $\tilde Z$. If $\pi$ were known, the ideal
\emph{relative leniency} instrument would be $\tilde \ell_i = \tilde z_i' \pi$, which measures
how lenient the assigned provider is relative to other providers who could have treated
patient $i$ \citep{GoldsmithPinkhamHullKolesarLeniencyManual}. Under random (or
as-good-as-random) provider assignment conditional on $w_i$ and standard exclusion
restrictions, using $\tilde \ell_i$ as an instrument in \eqref{eq:struct} yields an approximately
unbiased IV estimator even with many providers.

In practice $\pi$ is unknown. A naïve plug-in approach estimates $\pi$ by OLS and uses
the in-sample fitted values $\hat \ell_i = \tilde z_i' \hat \pi$ as the instrument. This is
numerically equivalent to conventional 2SLS with the full set of provider dummies and
suffers from many-instrument bias, because $\hat \ell_i$ is partially constructed from $x_i$
itself. UJIVE replaces $\hat \ell_i$ with a leave-one-out (jackknife) estimate that omits
observation $i$ from the first stage:
\[
  \hat \ell_{-i} = \tilde z_i' \hat \pi_{-i},
\]
where $\hat \pi_{-i}$ is the OLS coefficient from regressing $\tilde x$ on $\tilde Z$ using all
observations $j \neq i$. Under independent sampling, $\hat \pi_{-i}$ is unbiased for $\pi$
and, crucially, is independent of the pair $(\tilde z_i, \varepsilon_i)$, so that $\hat \ell_{-i}$ is
approximately uncorrelated with the structural error $\varepsilon_i$ even when there are
many providers. As shown by \citet{Kolesar2013Late}, this eliminates the leading
many-instrument bias term while preserving consistency for a convex combination of local
average treatment effects.

Computationally, the leave-one-out fitted values can be obtained without re-estimating the
first-stage regression $n$ times. Let $\hat \pi$ be the full-sample OLS coefficient from
regressing $\tilde x$ on $\tilde Z$, and let $\hat v_i$ and $h_{ii}$ denote the corresponding
residual and diagonal element of the projection (hat) matrix,
\[
  \hat v = \tilde x - \tilde Z \hat \pi, \qquad
  H = \tilde Z(\tilde Z'\tilde Z)^{-1}\tilde Z', \qquad h_{ii} = H_{ii}.
\]
Then the leave-one-out prediction for $\tilde x_i$ has the standard jackknife form
\[
  \hat \ell_{-i}
  = \tilde z_i' \hat \pi_{-i}
  = \tilde x_i - \frac{\hat v_i}{1 - h_{ii}}.
\]
We use $\hat \ell_{-i}$ as a single scalar instrument in a just-identified IV regression of
$y_i$ on $x_i$ with the same covariates $w_i$ as in \eqref{eq:struct}. In practice, we
implement this by: (i) residualizing $x_i$ and the provider indicators $z_i$ with respect to
the assignment controls $w_i$, (ii) computing $\hat \ell_{-i}$ via the leave-one-out formula
above, and (iii) estimating $\beta$ by just-identified IV with $x_i$ instrumented by
$\hat \ell_{-i}$ and including $w_i$ as regressors. We report heteroskedasticity-robust (HC1)
standard errors, which are typically appropriate in leniency designs when patient-level
observations are independent conditional on assignment controls
\citep{GoldsmithPinkhamHullKolesarLeniencyManual}.

In summary, UJIVE allows us to exploit quasi-random variation in provider ``batching
tendency'' without incurring the many-judge bias associated with conventional 2SLS using a
large set of provider dummies. This makes it a natural and robust estimator for our ED
leniency design with many physicians and rich time and case-mix controls.
...
Empirically, we verify that ``late batching"---cases where a batch follows a single test—is rare, occurring in only 189 encounters of our sample (1.91\% of multi-test encounters). To verify this classification does not impact our results, we re-estimated all models treating such cases as batched (Appendix D). The results are virtually identical.
...
To further separate clinical decision-making time from test completion time, we examine ``time to disposition"---the duration until physicians make admission/discharge decisions, excluding post-decision boarding. Additionally, per a request from Reviewer 3, we also examime ``treatment time"---defined as the time from when the patient is roomed in the ED to when a disposition decision is made. Our estimates show that batching has meaningfully large and significant effects on both outcomes, demonstrating that early batching affects not only mechanical test completion waiting but also clinical processing efficiency, since these dispositions decisions can be made before the results of the imaging study have been read by a radiologist. Even when excluding post-disposition time (during which no testing occurs), the early discretionary batching decision yields substantial point estimates for delays in clinical decision-making.
...
Although including physician fixed effects in Equation (1) would eliminate all identifying variation (as discussed above), we fully agree with the reviewer’s underlying point: any instrument based on between-physician variation should be examined for potential contamination by unobserved, physician-specific traits. In direct response to this suggestion, we implement the \emph{Unbiased Jackknife Instrumental Variables Estimator (UJIVE)} following \citet{goldsmithpinkham2025leniencydesignsoperatorsmanual}, which provides an appropriate diagnostic for leniency-design IVs when a large set of physician indicators can serve as instruments.
...
The table included below (now included in Appendix A) compares patient characteristics and outcomes between excluded and included encounters:
...
While our results apply to the 25\% of ED visits requiring imaging workups, this represents the clinically and operationally relevant population. Furthermore, this 25\% of ED visits consume 41\% of imaging resources in our data. Our focused approach identifies the precise effects where physician practice style influences operations---the margin where ED management interventions can meaningfully improve efficiency, and the margin where physicians disagree on optimal testing strategies.
...
Figure 4 (included) presents this heterogeneity analysis, with complaints ordered by batching rate. The color gradient illustrates variation in batching prevalence, from red (high-batching complaints) to blue (low-batching complaints).
